...
1. 「义务」页对同一付款方/收款方/币种/交割日录入两笔金额完全相同的 OPEN 义务
2. 「重复检测」页出现一组「待处理」疑似组，展示两条义务 ID、金额、创建时间
3. 「轧差执行」选择同交割日 + 币种：出现红色警告且「执行轧差」被禁用（后端同样拦截，返回 `DUPLICATE_SUSPECTED`）
4. 在「重复检测」页对其中一笔「标记重复并取消」（原因必填），或「整组已复核无问题」
5. 回到「轧差执行」，同条件轧差可正常执行完成
6. 已复核的组若再录入一条完全相同的 OPEN 义务，该组重新变为待处理并再次阻止轧差
7. `viewer` 登录可查看重复组，但看不到取消/复核操作（接口亦返回 403）
